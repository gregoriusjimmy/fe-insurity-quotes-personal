...
type CarAddressQuestionProps = {
  onAnswer: (data: TAddressForm  & {hasLivedAtAddressFor60Days:boolean}) => void;
  defaultValues?:TAddressForm
  
};

const CarAddressQuestion = ({ defaultValues, onAnswer }: CarAddressQuestionProps) => {
  const [hasLivedAtAddressFor60Days, setHasLivedAtAddressFor60Days] = useState<boolean>()
  const {
    register,
    handleSubmit,
    control,
  ...
  const onSubmit = useCallback((data:TAddressForm) => {
  ...
  return (
    <FormLayout
     ...
     <Controller
        ...
        <p className="font-bold mb-4">Have you lived at this address for at least 60 days?</p>
        <Button className="mb-4"  variant="outlined" active={hasLivedAtAddressFor60Days} onClick={()=>{setHasLivedAtAddressFor60Days(true)}} >Yes</Button>
        <Button className="mb-8"  variant="outlined" active={ hasLivedAtAddressFor60Days === false}  onClick={()=>{setHasLivedAtAddressFor60Days(false)}}>No</Button>
          <Button className="" onClick={handleSubmit(onSubmit)} disabled={!isValid || hasLivedAtAddressFor60Days === undefined}>Continue</Button>
          </div>
        </div>
      }
    />
