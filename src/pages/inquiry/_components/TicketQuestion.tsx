...
const [ticketForm,setTicketForm] = useState<TTicketForm>(TICKET_FORM_FACTORY)
  ...
  const handleDateChange = useCallback(
    (key: keyof TTicketForm, index: number, value: string) => {
      ...
      const formattedValue =
        cleanValue.length === 2 && !cleanValue.includes("/") ? `${cleanValue}/` : cleanValue.slice(0, 7);
  ...
  const hasErrors =useMemo(()=> Object.values(dateErrors).some((errors) =>
    errors.some((error) => error && error.length > 0)
  ),[dateErrors])
  
  const formIsEmpty = useMemo(()=> Object.values(ticketForm).every(
  ...
  return ( 
    ...
    intro="Let's talk driving history"
      ...
      addition={hasTickets &&<div className="flex flex-col justify-center w-full mt-8" >
        <div className="text-center text-sm mb-4">All good—it happens. How many?</div>
        {Object.keys(ticketForm).map((key,idx)=>(<div className="flex flex-col  pb-3 border-b items-center" key={idx}>
          <div className="flex justify-between pt-3 w-full">
          <div className="text-foreground-700">{key}</div>
          <div className="flex items-center space-x-4">
          <Minus onClick={()=>handleClickMinus(key as keyof TTicketForm)} className="text-foreground-700 w-6 h-6 cursor-pointer" />
          <span className="text-lg font-bold select-none">{ticketForm[key as keyof TTicketForm].count}</span>
          <Plus   onClick={()=>handleClickPlus(key as keyof TTicketForm)} className=  "text-primary-700 w-6 h-6 cursor-pointer"/>
          </div>
       
          </div>
          {Array.from({ length: ticketForm[key as keyof TTicketForm].count }, (_, index) => (
  <div className="w-full flex flex-col pt-4" key={index}>
    ...
    maxLength={7}
                        ...
                        onBlur={(e) => handleBlur(key as keyof TTicketForm, index, e.target.value)}
                        onChange={(e) =>
                          handleDateChange(
        ...
        <Button onClick={()=>onYes(ticketForm)} disabled={hasErrors || (hasTickets && formIsEmpty)}>Continue</Button>
        </div>}
    />
  )
}
